...
vis_magOTUs_df_numClusters_all_genus <- vis_magOTUs_df_all %>%
                        group_by(sample, Genus) %>%
                          summarise(sample, Host, number_of_clusters = n_distinct(Cluster), Genus = Genus, .groups="keep") %>%
                            unique()

vis_magOTUs_df_numClusters_all_plot_genus <- ggplot(vis_magOTUs_df_numClusters_all_genus, aes(x = factor(Host, levels = c("Apis florea", "Apis cerana", "Apis mellifera", "Apis dorsata")), y = number_of_clusters, fill = Host)) +
                                        geom_boxplot(outlier.shape = NA) +
                                        geom_jitter() +
                                        labs(y = "Number of magOTUs per individual", x = "Host species") +
                                          make_theme(leg_pos = "none", x_angle = 30, setFill = F, x_vj = 1, x_hj = 1, ) +
                                          scale_fill_manual(values=host_order_color) +
                                          facet_wrap(~ factor(Genus, genera))
                                            ggsave("Figures/09a-magOTU_number_per_sample_all_per_genus.pdf")
...
vis_magOTUs_df_numClusters_genus <- vis_magOTUs_df %>%
                        group_by(sample, Genus) %>%
                          summarise(sample, Host, number_of_clusters = n_distinct(Cluster), Genus = Genus) %>%
                            unique
...
vis_magOTUs_df_numClusters_passed_plot_genus <- ggplot(vis_magOTUs_df_numClusters_genus, aes(x = factor(Host, levels = c("Apis florea", "Apis cerana", "Apis mellifera", "Apis dorsata")), y = number_of_clusters, fill = Host)) +
                                        geom_boxplot(outlier.shape = NA) +
                                        geom_jitter() +
                                        labs(y = "Number of magOTUs per individual", x = "Host species") +
                                          make_theme(leg_pos = "none", x_angle = 30, setFill = F, x_hj = 1, x_vj = 1) +
                                          scale_fill_manual(values=host_order_color) +
                                          facet_wrap(~ factor(Genus, genera))
                                          ggsave("Figures/09a-magOTU_number_per_sample_passed_per_genus.pdf")
```
...
vis_magOTUs_df_numClusters_all_plot_genus
vis_magOTUs_df_numClusters_passed_plot_genus
```
