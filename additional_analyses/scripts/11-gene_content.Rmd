...
df_cazyme_counts_mag <- data.frame()
for (sample in samples_IN_MY) {
  ...
  df_cazyme_genus <- df_cazyme %>%
    ...
    summarize(count = sum(rpkm), .groups = "keep") %>%
    ungroup()
  df_cazyme_mag <- df_cazyme %>%
    group_by(cazyme, mag, sample) %>%
    summarize(count = sum(rpkm), .groups = "keep") %>%
    ungroup()
  ...
  df_cazyme_counts_mag <- rbind(df_cazyme_counts_mag, df_cazyme_mag)
}
# save the data
write.csv(df_cazyme_counts_sample, "additional_analyses/results/cazy_profiles/cazyme_counts_sample.csv", row.names = FALSE)
write.csv(df_cazyme_counts_species, "additional_analyses/results/cazy_profiles/cazyme_counts_species.csv", row.names = FALSE)
write.csv(df_cazyme_counts_genus, "additional_analyses/results/cazy_profiles/cazyme_counts_genus.csv", row.names = FALSE)
write.csv(df_cazyme_counts_mag, "additional_analyses/results/cazy_profiles/cazyme_counts_mag.csv", row.names = FALSE)
...
# write.csv(df_cazyme_counts_mag, "results/figures/08-summarize_functions/cazyme_counts_mag.csv", row.names = FALSE)
...
```{r}
# combine df_cazyme_counts_species with the presence or absence of the species in each sample

df_cazyme_counts_species <- read.csv("results/figures/08-summarize_functions/cazyme_counts_species.csv")
df_abundance_table <- df_plot %>%
                        select(sample, MAG_species_name_final, Genus)

# plot cazymes for each Species on the y axis and cazyme family on the x axis and size showing the proportion of number of MAGs of the species or if x axis is mag, the number of copies of the gene family (cazy) in the MAG
# show colors bars at the bottom for the substrate
...
cazyme_prevalence_species <- df_abundance_table %>%
                              group_by(MAG_species_name_final) %>%
                              reframe(MAG_species_name_final, Genus, num_samples = n_distinct(sample)) %>%
                              # filter(num_samples >= 5) %>%
                              unique() %>%
                              right_join(df_cazyme_counts_species, by = c("MAG_species_name_final" = "species")) %>%
                              group_by(cazyme, MAG_species_name_final) %>%
                              reframe(count_cazyme_samples = sum(count > 1), num_samples, Genus) %>%
                              unique() %>%
                              mutate(cazyme_discovered_percent = count_cazyme_samples/num_samples*100) %>%
                              left_join(glycan_annotations_cleaned_filt %>%
                                          select(Subfamily, FUNCTION_AT_DESTINATION_3)
                                      , by = c("cazyme" = "Subfamily")) %>%
                              filter(grepl("ectin|ellulose|emicellulose", FUNCTION_AT_DESTINATION_3))
# from saved R data, df_rohdes
# write.csv(df_rohdes, "additional_analyses/results/cazy_profiles/rohdes_data.csv")
df_rohdes <- read.csv("additional_analyses/results/cazy_profiles/rohdes_data.csv") %>% filter(type == "prev")
df_host_species_preval <- read.csv("results/figures/magotu_ab_ranks.csv") %>%
                            left_join(df_rohdes %>% select(X, X_old), by = c("X" = "X_old")) %>%
                            rename(MAG_species_name_final = X.y) %>% unique()
get_host_species_of_species <- function(species) {
  # print(species)
  rohdes_index <- df_rohdes %>%
    filter(MAG_species_name_final == species) %>%
    filter(type == "prev") %>%
    pull(rohdes_index)
  # if rohdes_index is numeric(0) return NA
  if (length(rohdes_index) == 0) {
    return(NA)
  }
  if (length(rohdes_index != 1) != 1) {
    stop(paste0("Problem chacking rohdes index for ", species))
  }
  if (rohdes_index != 1) {
    df_spec <- df_host_species_preval %>%
      filter(MAG_species_name_final == species)
    
    h_1 <- colnames(df_spec)[which(df_spec == 1)]
    h_2 <- colnames(df_spec)[which(df_spec == 2)]
    
    if (h_1 %in% c("florea", "andreniformis") & h_2 %in% c("florea", "andreniformis")) {
      return("Dwarf bees")
    } else {
      return("Mixed")
    }
    ...
    host_species <- colnames(df_spec)[which(df_spec == 1)]
    return(paste0("Apis ", host_species))
  }
}
# view(cazyme_prevalence_species)
# 5 or more cazymes and at least 1 pectin and num_samples >= 5
cazyme_genera <- cazyme_prevalence_species %>%
                  group_by(Genus) %>%
                  mutate(n_cazymes = n_distinct(cazyme)) %>%
                  filter(n_cazymes >= 5) %>%
                  # # filter(any(FUNCTION_AT_DESTINATION_3 == "Pectin")) %>%
                  filter(num_samples >= 5) %>%
                  filter(Genus != "NA") %>%
                  filter(!Genus %in% c("g__Pantoea", "g__Klebsiella")) %>%
                  arrange(desc(num_samples), desc(n_cazymes)) %>%
                  pull(Genus) %>% unique()

# cazyme_genera <- c("g__Lactobacillus",
#                   "g__Bifidobacterium",
#                   "g__Gilliamella",
#                   "g__Dysgonomonas",
#                   "g__Bartonella_A",
...
#                   "g__WRHT01",
#                   "g__Pluralibacter")
cazyme_prevalence_species <- cazyme_prevalence_species %>%
                              left_join(df_rohdes %>% select(MAG_species_name_final, rohdes_index), by = c("MAG_species_name_final")) %>%
                              filter(Genus %in% cazyme_genera) %>%
                              filter(!is.na(rohdes_index)) %>%
                              mutate(host_species = Vectorize(get_host_species_of_species)(MAG_species_name_final)) %>%
                              mutate(FUNCTION_AT_DESTINATION_3 = ifelse(FUNCTION_AT_DESTINATION_3 %in% c("Pectin"#,
                                                                                                        #  "Hemicellulose",
                                                                                                        #  "Hemicellulose,Pectin",
                                                                                                        #  "Amylose_and_Amylopectin"
                                                                                                        #  "Gum,Pectin",
                                                                                        ), FUNCTION_AT_DESTINATION_3, "Other"))
ggplot() +
  geom_point(data = cazyme_prevalence_species,
    aes(y = cazyme,
        x = MAG_species_name_final,
        size = cazyme_discovered_percent,
        color = host_species)) +
  scale_size_continuous(range = c(0.5, 4)) +
  # continuous color scale manually defined
  scale_color_manual(values = extend_colors(names_vec = unique(cazyme_prevalence_species$host_species),
                                            colors_vec = c(host_order_color_dark, "Dwarf bees" = "#a65628"))
                                            ) +
  # scale_color_gradient2(low = "#f7fbff", mid = "#4292c6", high = "#08306b", midpoint = 0.7) +
  # scale_color_manual(values = extend_colors(names_vec = unique(cazyme_prevalence_species$FUNCTION_AT_DESTINATION_3),
  #                                           colors_vec = c("Pectin" = "#e31a1c",
  #                                                         "Amylose_and_Amylopectin" = "#cab2d6",
  #                                                         # "Gum,Hemicellulose,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Gum,Pectin"
  #                                                         # "Glycoprotein,Gum,Hemicellulose,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Hemicellulose,Mannans_and_Heteromannans"
  #                                                         # "Gum,Glycoprotein,Pectin"
  #                                                         # "Hemicellulose,Pectin"
  #                                                         # "Amylose_and_Amylopectin,Gum,Hemicellulose,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Cellulose,Gum,Glycoprotein,Hemicellulose,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Amylose_and_Amylopectin,Gum,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Gum,Glycoprotein,Hemicellulose,Pectin"
  #                                                         # "Amylose_and_Amylopectin,Pectin"
  #                                                         # "Cellulose,Glycoprotein,Gum,Hemicellulose,Mannans_and_Heteromannans,Pectin"
  #                                                         # "Amylose_and_Amylopectin,Gum,Pectin"
  #                                                         # "Cellulose,Hemicellulose,Mannans_and_Heteromannans"
  #                                                          "Hemicellulose" = "#045a8d",
  #                                                          "Cellulose" = "#33a02c")
  #                                           )
  #                                 ) +
  facet_grid(FUNCTION_AT_DESTINATION_3~factor(Genus, cazyme_genera), scales = "free", space='free_x') +
  labs(y = "CAZyme", x = "Species", size = "Percentage of samples containing CAZyme within bacterial species (%)", color = "Specificity") +
  # add x axis and y axis grid lines
  make_theme(setFill = FALSE, setCol = FALSE, leg_pos = "right",
              x_angle = 45, x_vj = 1, x_hj = 1, modify_guide = F,
              x_size = 8,
              y_size = 8, y_hj = 1) +
  theme(panel.grid.major.x = element_line(color = "grey", size = 0.5),
        panel.grid.major.y = element_line(color = "grey", size = 0.5),
        panel.grid.minor.x = element_line(color = "grey", size = 0.5),
        panel.grid.minor.y = element_line(color = "grey", size = 0.5))
ggsave("additional_analyses/figures/cazyme_discovery_dot_plot_color_dwarf.pdf", width = 20, height = 15)
# ggsave("additional_analyses/figures/cazyme_discovery_dot_plot_color.pdf", width = 20, height = 15)
# ggsave("additional_analyses/figures/cazyme_discovery_dot_plot.pdf", width = 20, height = 15)
...
# plot pathway (completeness?) from MAGs identified by minpath.
df_mag_list <- read.csv("additional_analyses/results/functional_comparison/08-summarize_functions/all_mag_names.csv", header=FALSE)
colnames(df_mag_list) <- c("mag_name", "species_name", "file_name","genus")
mag_names = df_mag_list %>% pull(mag_name) %>% unique

# df_minpath_outs <- data.frame()
# for (mag_name_iter in mag_names) {
#   file_name <- df_mag_list %>% filter(mag_name == mag_name_iter) %>% pull(file_name) %>% unique
#   if (!file.exists(paste0("additional_analyses/results/functional_comparison/08-summarize_functions/minpath_outputs/by_mag_renamed/",file_name,".minpath.parsed"))) {
#     print(paste0("cannnot find ", file_name))
#     next
#   }
#   df_minpath_iter <- read.csv(paste0("additional_analyses/results/functional_comparison/08-summarize_functions/minpath_outputs/by_mag_renamed/",file_name,".minpath.parsed"), sep = "\t" , header = F)
#   colnames(df_minpath_iter) <- c("pathway_id", "naive_name", "naive", "minpath_name", "minpath", "fam0_name", "fam0", "fam_found_name", "fam_found", "name_name", "pathway_name")
#   df_minpath_iter <- df_minpath_iter %>%
#                       select(!c("naive_name", "minpath_name", "fam0_name", "fam_found_name", "name_name")) %>%
#                       select(pathway_name, minpath, fam_found) %>%
#                       mutate(mag = mag_name_iter)
#                       # mutate(spec = spec_name)
#   df_minpath_outs <- rbind(df_minpath_outs, df_minpath_iter)
# }
# write.csv(df_minpath_outs, "additional_analyses/results/functional_comparison/08-summarize_functions/minpath_outputs/all_minpath_outs.csv", row.names = F)
df_minpath_outs <- read.csv("additional_analyses/results/functional_comparison/08-summarize_functions/minpath_outputs/all_minpath_outs.csv")

df_minpath_outs_plot <- df_minpath_outs %>%
                          select(pathway_name, minpath, mag) %>%
                          left_join(df_mag_list, by = c("mag" = "mag_name")) %>%
                          group_by(pathway_name) %>%
                          mutate(count = sum(minpath)) %>%
                          filter(count > 0) %>%
                          select(!count) %>%
                          ungroup() %>%
                          left_join(vis_magOTUs_df %>% ungroup %>% select(ID, Quality), by = c("mag" = "ID")) %>%
                          filter(Quality == "high") %>%
                          select(!Quality) %>%
                          group_by(pathway_name) %>%
                          mutate(count = sum(minpath)) %>%
                          ...
                          mutate(count = n_distinct(mag)) %>%
                          filter(count > 2) %>%
                          select(!count) %>%
                          ungroup() %>%
                          mutate(minpath = as.factor(minpath)) %>%
                          left_join(kegg_path_info, by = c("pathway_name" = "path_name")) %>%
                          filter(subsection %in% c("1.1 Carbohydrate metabolism"))

df_minpath_outs_mat <- df_minpath_outs_plot %>%
                        select(pathway_name, minpath, mag) %>%
                        pivot_wider(id_cols = c(mag), names_from = pathway_name, values_from = minpath) %>%
                        column_to_rownames("mag") %>%
                        # replace NAs with 0
                        mutate_all(~replace(., is.na(.), 0)) %>%
                        as.matrix() #%>% t()

section_names <- colnames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(kegg_path_info, by = c("." = "path_name")) %>% pull(section)
subsection_names <- colnames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(kegg_path_info, by = c("." = "path_name")) %>% pull(subsection)
mag_species_names <- rownames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(species_name)
mag_genus_names <- rownames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(genus)
dominant_host_names <- rownames(df_minpath_outs_mat) %>% as.data.frame() %>%
                          left_join(vis_magOTUs_df %>% ungroup %>% select(ID, magOTU), by = c("." = "ID")) %>%
                          left_join(handmade_species_names %>% select(cluster, MAG_species_name_final), by = c("magOTU" = "cluster")) %>%
                            mutate(host = Vectorize(get_host_species_of_species)(MAG_species_name_final)) %>% pull(host) %>% as.character()

right_annotation <- rowAnnotation(
  genus = mag_genus_names,
  col = list(genus = unlist(genusColorsClean))
)

left_annotation <- rowAnnotation(
  host_species = dominant_host_names,
  col = list(host_species = c(host_order_color_dark, "Mixed" = "#999999", "Dwarf bees" = "#a65628"))
  )

rightleft_annotation <- rowAnnotation(
  genus = mag_genus_names,
  host_species = dominant_host_names,
  col = list(genus = unlist(genusColorsClean),
             host_species = c(host_order_color_dark, "Mixed" = "#999999", "Dwarf bees" = "#a65628"))
  )

top_annotation <- HeatmapAnnotation(
  section = section_names,
  col = list(section = make_col_list(section_names)
  )
  )

row_split_vec <- as.character(as.factor(mag_species_names))
col_split_vec <- as.character(as.factor(subsection_names))
# unique(col_split_vec)
# names_ordered <- c(
#   ""
# )
# col_split_vec <- as.character(as.factor(mag_species_names, levels = names_ordered))
# make a heatmap
heatmap_obj <- Heatmap(
  df_minpath_outs_mat,
  name = "Pathway_presence",
  col = c("0" = "#ffffff", "1" = "#045a8d"),
...
# pdf("additional_analyses/figures/minpath_presence_heatmap_all_snod_saez_gilli_entomo.pdf", width = 30, height = 20)
pdf("additional_analyses/figures/minpath_presence_heatmap_all_genera.pdf", width = 20, height = 20)
...
df_minpath_outs_plot <- df_minpath_outs %>%
                          ...
                          filter(Quality == "high") %>%
                          select(!Quality) %>%
                          filter(genus %in% c("Saezia", "Entomomonas", "Snodgrassella", "Gilliamella")) %>%
                          # filter(genus %in% c("Saezia", "Entomomonas", "Snodgrassella", "Gilliamella", "Lactobacillus", "Bifidobacterium")) %>%
                          group_by(pathway_name) %>%
                          mutate(count = sum(minpath)) %>%
                          filter(count > 0) %>%
                          select(!count) %>%
...
# ggplot() +
#   # geom_point(data = df_minpath_outs_plot, aes(x = mag, y = pathway_name, color = minpath)) +
#   geom_tile(data = df_minpath_outs_plot, aes(x = mag, y = pathway_name, fill = minpath)) +
#   # treat na as 0
#   scale_fill_manual(values = c("0" = "#ffffff", "1" = "#045a8d")) +
#   facet_grid(subsection~genus, scales = "free", space = "free") +
#   labs(x = "MAG", y = "Pathway", fill = "Completeness") +
#   make_theme(setFill = FALSE, setCol = FALSE, leg_pos = "right",
#               x_angle = 45, x_vj = 1, x_hj = 1, modify_guide = F,
#               x_size = 8,
#               y_size = 8, y_hj = 1)
# ggsave("additional_analyses/figures/minpath_presence_tile_plot.pdf", width = 25, height = 30)
# ggsave("additional_analyses/figures/minpath_presence_tile_plot.pdf", width = 25, height = 30)

# make a matrix and send to complex heatmap
...
section_names <- rownames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(kegg_path_info, by = c("." = "path_name")) %>% pull(section)
subsection_names <- rownames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(kegg_path_info, by = c("." = "path_name")) %>% pull(subsection)
mag_species_names <- colnames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(species_name)
mag_genus_names <- colnames(df_minpath_outs_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(genus)
dominant_host_names <- colnames(df_minpath_outs_mat) %>% as.data.frame() %>%
                          left_join(vis_magOTUs_df %>% ungroup %>% select(ID, magOTU), by = c("." = "ID")) %>%
                          left_join(handmade_species_names %>% select(cluster, MAG_species_name_final), by = c("magOTU" = "cluster")) %>%
                            mutate(host = Vectorize(get_host_species_of_species)(MAG_species_name_final)) %>% pull(host) %>% as.character()
...
topbot_annotation <- HeatmapAnnotation(
  genus = mag_genus_names,
  host_species = dominant_host_names,
  col = list(genus = unlist(genusColorsClean),
             host_species = c(host_order_color_dark, "Mixed" = "#999999", "Dwarf bees" = "#a65628"))
...
row_split_vec <- as.character(as.factor(subsection_names))
col_split_vec <- as.character(as.factor(mag_species_names))
# unique(col_split_vec)
# names_ordered <- c(
#   ""
# )
...
heatmap_obj <- Heatmap(
  ...
  column_split = col_split_vec,
  row_split = row_split_vec,
  bottom_annotation = topbot_annotation,
  top_annotation = topbot_annotation,
  right_annotation = right_annotation,
  cluster_rows = T,
  cluster_columns = T,
  show_column_dend = F,
...
# pdf("additional_analyses/figures/minpath_presence_heatmap_all_snod_saez_gilli_entomo.pdf", width = 30, height = 20)
pdf("additional_analyses/figures/minpath_presence_heatmap_snod_saez_gilli_entomo.pdf", width = 30, height = 20)
draw(heatmap_obj, heatmap_legend_side = "right")
dev.off()
...
df_minpath_outs_plot <- df_minpath_outs %>%
                          ...
                          filter(Quality == "high") %>%
                          select(!Quality) %>%
                          filter(genus %in% c("Bombilactobacillus", "Bifidobacterium", "Lactobacillus", "Gilliamella")) %>%
                          # filter(genus %in% c("Saezia", "Entomomonas", "Snodgrassella", "Gilliamella", "Lactobacillus", "Bifidobacterium")) %>%
                          group_by(pathway_name) %>%
                          mutate(count = sum(minpath)) %>%
                          filter(count > 0) %>%
...
# pdf("additional_analyses/figures/minpath_presence_heatmap_all_core.pdf", width = 30, height = 20)
pdf("additional_analyses/figures/minpath_presence_heatmap_core.pdf", width = 30, height = 20)
draw(heatmap_obj, heatmap_legend_side = "right")
dev.off()
...
df_minpath_outs_plot <- df_minpath_outs %>%
                          ...
                          filter(genus %in% c("Lactobacillus", "Bifidobacterium", "Bombilactobacillus",
                                              "Snodgrassella", "Gilliamella", "Bartonella", "Apibacter",
                                              "Frischella", "Commensalibacter", "Apilactobacillus"
                          )) %>%
                          group_by(pathway_name) %>%
                          mutate(count = sum(minpath)) %>%
                          filter(count > 0) %>%
...
# pdf("additional_analyses/figures/minpath_presence_heatmap_all_core_extended.pdf", width = 30, height = 20)
pdf("additional_analyses/figures/minpath_presence_heatmap_core_extended.pdf", width = 30, height = 20)
draw(heatmap_obj, heatmap_legend_side = "right")
dev.off()

write.csv(df_minpath_outs_plot, "additional_analyses/results/functional_comparison/08-summarize_functions/minpath_outputs/all_minpath_outs_plot.csv", row.names = F)
...
df_counts_mat <- df_minpath_outs %>%
                    ...
                    mutate_all(~replace(., is.na(.), 0)) %>%
                    as.matrix()
input_data <- df_counts_mat
meta_table <- data.frame(Species = rownames(df_counts_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(species_name))
meta_table$Species <- factor(meta_table$Species, levels = sort(unique(meta_table$Species)))
rownames(meta_table) <- rownames(df_counts_mat)
input_metadata <- meta_table
saveRDS(input_data, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_data.rds")
saveRDS(input_metadata, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_metadata.rds")

# input_data <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_data.rds")
# input_metadata <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_metadata.rds")
# fit_data = Maaslin2(
#     input_data = input_data, 
#     input_metadata = input_metadata, 
#     output = "additional_analyses/results/functional_comparison/08-summarize_functions//maaslin2_results_minpath", 
#     analysis_method = "LM", # consider
#     transform = "LOGIT", # consider
...
models_data <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath/fits/models.rds")
...
write.csv(df_pairwise, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_parsed/maaslin2_results_minpath.csv", row.names = F, quote = F)
...
df_mag_list <- read.csv("additional_analyses/results/functional_comparison/08-summarize_functions/all_mag_names.csv", header=FALSE)
colnames(df_mag_list) <- c("mag_name", "species_name", "file_name","genus")
meta_table <- data.frame(Genus = rownames(df_counts_mat) %>% as.data.frame() %>% left_join(df_mag_list, by = c("." = "mag_name")) %>% pull(genus))
meta_table$Genus <- factor(meta_table$Genus, levels = sort(unique(meta_table$Genus)))
rownames(meta_table) <- rownames(df_counts_mat)
input_metadata <- meta_table
saveRDS(input_data, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_by_genus_data.rds")
saveRDS(input_metadata, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_by_genus_metadata.rds")

# input_data <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_by_genus_data.rds")
# input_metadata <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_input_by_genus_metadata.rds")
# fit_data = Maaslin2(
#     input_data = input_data, 
#     input_metadata = input_metadata, 
#     output = "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_by_genus", 
#     analysis_method = "LM", # consider
#     transform = "LOGIT", # consider
#     normalization = "TSS", # consider
...
df_pairwise <- data.frame()
models_data <- readRDS("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_by_genus/fits/models.rds")
for (i in 1:length(models_data)) {
  model <- models_data[[i]]
  conts <- summary(lsmeans(model, pairwise ~ Genus, adjust = "FDR"))
  df_to_add <- data.frame(
                    feature = names(models_data)[[i]],
                    contrast = conts$contrasts$contrast,
                    p.value = conts$contrasts$p.value,
...
write.csv(df_pairwise, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_by_genus_parsed/maaslin2_results_minpath.csv", row.names = F, quote = F)

df_sig_results <- read.csv("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath/significant_results.tsv", sep = "\t") %>%
                    left_join(kegg_path_info %>% mutate(path_name = make.names(path_name)), by = c("feature" = "path_name"))
write.csv(df_sig_results, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath/significant_results_annotated.csv", row.names = F, quote = F)
df_sig_results_by_genus <- read.csv("additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_by_genus/significant_results.tsv", sep = "\t") %>%
                    left_join(kegg_path_info %>% mutate(path_name = make.names(path_name)), by = c("feature" = "path_name"))
write.csv(df_sig_results_by_genus, "additional_analyses/results/functional_comparison/08-summarize_functions/maaslin2_results_minpath_by_genus/significant_results_annotated.csv", row.names = F, quote = F)
...
rpkm_matrix <- ifelse(rpkm_matrix > 0, 1, 0)
dist_matrix <- vegdist(rpkm_matrix, method = "jaccard")
...
df_rpkms_by_func_all <- read.csv("results/figures/08-summarize_functions/rpkm_matrix_by_function/rpkm_matrix_abs_all_kos.csv")
# include only carbohydrate metabolism genes
kos_to_include <- ko_category_info %>%
                    filter(B %in% c("Carbohydrate metabolism")) %>%
                    pull(ko)
df_rpkms_by_func <- df_rpkms_by_func_all %>%
                      filter(mag %in% mags_to_select) %>%
                      # select columns in kos_to_include
                      select(mag, one_of(kos_to_include))
ani_matrix <- ani_matrix[mags_to_select, mags_to_select]
rpkm_matrix <- df_rpkms_by_func %>% column_to_rownames("mag") %>% as.matrix()
dist_matrix <- vegdist(rpkm_matrix, method = "robust.aitchison")
matrix <- as.matrix(dist_matrix)
...
ggplot(df_pcoa_new) +
    ...
    leg_pos = "bottom", leg_size = 8, guide_nrow = 30,
    x_size = 10, y_size = 10, modify_guide = F)
    ggsave("additional_analyses/figures/all_carbohydrate_metabolism_kos_abs_genus_circle_spec_marked.pdf", width = 15, height = 15)
...
# Need to correct this!
ko_category_info <- read.csv("results/figures/08-summarize_functions/kegg_info_dict.txt", sep = "\t") %>%
                      filter(!grepl("carcinoma", B)) %>%
                      filter(!grepl("Carcinoma", B)) %>%
                      filter(!grepl("cancer", B)) %>%
                      filter(!grepl("Cancer", B)) %>%
                      filter(!grepl("diabet", B)) %>%
                      filter(!grepl("Diabet", B))
cat_A_names <- unique(ko_category_info$A)
cat_B_names <- unique(ko_category_info$B)
kos_to_exclude <- ko_category_info %>%
                    filter(A %in% c("Human Diseases", "Organismal Systems")) %>%
                    # filter(A %in% c("Metabolism", "Genetic Information Processing", "Environmental Information Processing")) %>%
                    pull(ko)
all_mag_metadata <- read.csv("results/figures/magOTU_metadata_full_exported.csv") %>%
                      ...
                      mutate(Location = Vectorize(get_location_from_sample_name)(Sample)) %>%
                      mutate(Genus = Vectorize(clean_g_name)(Genus))
mags_to_select <- all_mag_metadata %>%                 # filter(Genus %in% c("Snodgrassella", "Saezia", "Gilliamella")) %>%
                      filter(Genus %in% c("Snodgrassella", "Entomomonas", "Saezia", "Gilliamella")) %>%
                      filter(Completeness > 90) %>%
                      filter(Quality == "high") %>%
                      # filter(Representative == "1") %>%
                      pull(ID)

fold_long_string <- function(string, n=150) {
  return(paste(strwrap(string, n), collapse="\n"))
}

# df_rpkms_by_func_all <- read.csv("results/figures/08-summarize_functions/rpkm_matrix_by_function/rpkm_matrix_abs_all_kos.csv")
df_rpkms_by_func_all <- read.csv("results/figures/08-summarize_functions/rpkm_matrix_by_function/rpkm_matrix_abs_all_kos_duped.csv")
for (name in cat_A_names) {
  rpkm_matrix <- df_rpkms_by_func_all %>%
                          ...
                          as.matrix()
  # rpkm_matrix <- rpkm_matrix[, !colnames(rpkm_matrix) %in% kos_to_exclude]
  if (name %in% c("Human Diseases", "Organismal Systems")) {
    next
  }
  cat_A_kos <- ko_category_info %>%
                  filter(A == name) %>%
                  pull(ko_uniq)
  cat_A_kos <- cat_A_kos[cat_A_kos %in% colnames(rpkm_matrix)]
  rpkm_matrix <- rpkm_matrix[, cat_A_kos]
  # make it into presence absence
  rpkm_matrix <- ifelse(rpkm_matrix > 0, 1, 0)
  # only keep kos found in 90% mags across each species
  snod_mags <- rownames(rpkm_matrix) %>% as.data.frame() %>%
                  left_join(all_mag_metadata, by = c(. = "ID")) %>%
                      filter(Quality != "low") %>%
                      filter(Genus %in% c("Snodgrassella")) %>%
                      pull(".")
  ento_mags <- rownames(rpkm_matrix) %>% as.data.frame() %>%
                  left_join(all_mag_metadata, by = c(. = "ID")) %>%
                      filter(Quality != "low") %>%
                      filter(Genus %in% c("Entomomonas")) %>%
                      pull(".")
  saezia_mags <- rownames(rpkm_matrix) %>% as.data.frame() %>%
                  left_join(all_mag_metadata, by = c(. = "ID")) %>%
                      filter(Quality != "low") %>%
                      filter(Genus %in% c("Saezia")) %>%
                      pull(".")
  gilli_mags <- rownames(rpkm_matrix) %>% as.data.frame() %>%
                  left_join(all_mag_metadata, by = c(. = "ID")) %>%
                      filter(Quality != "low") %>%
                      filter(Genus %in% c("Gilliamella")) %>%
                      pull(".")
  kos_to_keep <- colnames(rpkm_matrix) %>% as.data.frame() %>%
                  left_join(ko_category_info, by = c(. = "ko_uniq")) %>%
                  filter(B == "Carbohydrate metabolism") %>%
                  pull(".") %>%
                  unique()
  if (length(kos_to_keep) == 0) {
    next
  }
  # # kos present in 90% of snod mags and ento mags and saezia mags
                                      # colSums(rpkm_matrix[gilli_mags,]) > 0.9 * length(gilli_mags)]
  # # kos present in 90% of snod mags and ento mags and saezia mags
  kos_to_keep <- colnames(rpkm_matrix)[colSums(rpkm_matrix[snod_mags,]) > 0.9 * length(snod_mags) |
                                      colSums(rpkm_matrix[ento_mags,]) > 0.9 * length(ento_mags) |
                                      colSums(rpkm_matrix[saezia_mags,]) > 0.9 * length(saezia_mags)]
                                      # colSums(rpkm_matrix[gilli_mags,]) > 0.9 * length(gilli_mags)]
  rpkm_matrix <- rpkm_matrix[, kos_to_keep]
  species_names = rownames(rpkm_matrix) %>% as.data.frame() %>%
                            left_join(all_mag_metadata %>% ungroup(), by = c(. = "ID")) %>%
                              pull(MAG_species_name_final)
  genus_names = rownames(rpkm_matrix) %>% as.data.frame() %>%
  ...
  ko_names <- colnames(rpkm_matrix)
  category_A <- data.frame(ko = colnames(rpkm_matrix)) %>%
                  left_join(ko_category_info, by = c("ko" = "ko_uniq")) %>%
                  group_by(ko) %>%
                  summarise(A = paste(unique(A), collapse = "|")) %>%
                  pull(A)
  category_B <- data.frame(ko = colnames(rpkm_matrix)) %>%
                  left_join(ko_category_info, by = c("ko" = "ko_uniq")) %>%
                  group_by(ko) %>%
                  summarise(B = paste(unique(B), collapse = "|")) %>%
                  mutate(B = Vectorize(fold_long_string)(B)) %>%
                  pull(B)
  category_C <- data.frame(ko = colnames(rpkm_matrix)) %>%
                  left_join(ko_category_info, by = c("ko" = "ko_uniq")) %>%
                  group_by(ko) %>%
                  summarise(C = paste(unique(C), collapse = "|")) %>%
                  mutate(C = Vectorize(fold_long_string)(C)) %>%
                  pull(C)
  desc_names <- data.frame(ko = colnames(rpkm_matrix)) %>%
                  left_join(ko_category_info, by = c("ko" = "ko_uniq")) %>%
                  group_by(ko) %>%
                  summarise(D = paste(unique(D), collapse = "|")) %>%
                  mutate(D = Vectorize(fold_long_string)(D)) %>%
                  pull(D)
  # desc_names <- data.frame(ko = colnames(rpkm_matrix)) %>%
  #                 left_join(df_ko_info_anno_sub, by = c(ko = "ko_id")) %>%
  #                 group_by(ko) %>%
  ...
  # col_split = specific_or_not %>% as.factor
  # col_split = species_names %>% as.factor
  col_split = genus_names %>% as.factor
  row_split = category_C %>% as.factor
  ...
  heatmap_obj = Heatmap(t(rpkm_matrix),
              ...
              row_title_gp = grid::gpar(fontsize = 8),
              border = T,
              # clustering "within" split rows
              cluster_row_slices = T,
              # row_dend_reorder = T,
              cluster_rows = T
              )
  pdf(paste0("additional_analyses/figures/ko_heatmaps/", name, "_heatmap_carbohydrate_metab_90.pdf"), width = 20, height = 30)
  draw(heatmap_obj, merge_legend = TRUE)
  dev.off()
}
species_names = rownames(rpkm_matrix) %>% as.data.frame() %>%
...
pdf(paste0("additional_analyses/figures/tax_func_heatmaps/ko_heatmap_ko_snod_seaz.pdf"), width = 20, height = 30)
draw(heatmap_obj, merge_legend = TRUE)
dev.off()
mags_to_select <- all_mag_metadata %>%
                      filter(Quality != "low") %>%
                      # filter(Genus %in% c("Snodgrassella", "Saezia")) %>%
                      filter(Genus %in% c("Snodgrassella", "Saezia", "Entomomonas")) %>%
                      # filter(Completeness > 90) %>%
                      # filter(Quality == "high") %>%
                      # filter(Representative == "1") %>%
                      pull(ID)
rpkm_matrix <- df_rpkms_by_func_all %>%
                        ...
                        as.matrix()
rpkm_matrix <- rpkm_matrix[, !colnames(rpkm_matrix) %in% kos_to_exclude]
# # keep only those shared by all mags (90%)
# shared_kos <- colnames(rpkm_matrix)[colSums(rpkm_matrix > 0) >= nrow(rpkm_matrix)*0.9]
shared_kos <- c()
metadata_sub <- all_mag_metadata %>%
                  filter(ID %in% mags_to_select)

shared_dict <- list()

# Get unique species names
rpkm_matrix <- rpkm_matrix[, colnames(rpkm_matrix) %in% shared_kos]
species_names = rownames(rpkm_matrix) %>% as.data.frame() %>%
                            left_join(all_mag_metadata %>% ungroup(), by = c(. = "ID")) %>%
                              pull(MAG_species_name_final)
host_names = rownames(rpkm_matrix) %>% as.data.frame() %>%
...
pdf(paste0("additional_analyses/figures/tax_func_heatmaps/ko_heatmap_ko_snod_commensalibacter.pdf"), width = 20, height = 30)
draw(heatmap_obj, merge_legend = TRUE)
dev.off()
# pdf(paste0("results/figures/11-figures/tax_func_heatmaps/ko_sig_heatmap_enriched_ko_list.pdf"), width = 20, height = 30)
# draw(heatmap_obj, merge_legend = TRUE)
...
Run minpath on Snodgrassella, Saezia and Entomomonas and compare against Gilliamella (a fellow gammaproteobacteria member) to understand what functions they are likely sharing so well.
