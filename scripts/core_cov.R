...
coord_table_header <- c("magOTU", "Sample", "Cov","PTR")
coord_table <- data.frame(magOTU=character(),Sample=character(),Cov_ter=numeric(),Ptr=numeric(),stringsAsFactors=FALSE)
if (do_plots == 1) {
...
for (magOTU_selected in magOTUs) {
    ...
    magOTU_coord_df <- data.frame(rep(magOTU_selected, length(samples)), samples, magOTU_cov, magOTU_ptr)
    names(magOTU_coord_df) <- coord_table_header
    coord_table <- rbind(coord_table, magOTU_coord_df)
}
if (do_plots == 1) {
    # dev.off()
