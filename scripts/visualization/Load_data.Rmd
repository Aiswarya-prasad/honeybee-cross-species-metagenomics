...
instrain_breadth_depth <- instrain_genomes_info_df %>%
                            ...
                            reframe(coverage, Host, breadth, length, sample, Genus, filtered_read_pair_count, 
                                      reads_unfiltered_pairs, reads_unfiltered_reads) %>%
                            filter(breadth > 0.5) %>%
                              mutate(coverage_factor = cut(coverage ,breaks=c(-1, 1e-04, 1e-03, 1e-02, 1e-01, 1, 10, 100, 1000, Inf),labels = c("<0.0001", "0.0001 - 0.001", "0.001 - 0.01", "0.01 - 0.1", "0.1 - 1", "1 - 10", "10 - 100", "100 - 1000", ">1000"))) %>%
...
instrain_breadth_depth_unfiltered <- instrain_genomes_info_df %>%
                            ...
                            reframe(coverage, Host, breadth, length, sample, Genus, filtered_read_pair_count, 
                              ...
                              ungroup()
instrain_prev <- instrain_breadth_depth_unfiltered %>%
                    mutate(detected = ifelse(coverage > 10 & breadth > 0.5, 1, 0)) %>%
...
instrain_prev_by_genus <- instrain_breadth_depth_unfiltered %>%
                    mutate(detected = ifelse(coverage > 10 & breadth > 0.5, 1, 0)) %>%
                    group_by(Host, Genus) %>%
                    mutate(detection_genus = ifelse(sum(detected) > 1, 1, 0)) %>%
                    mutate(presence_in_host = mean(detection_genus)) 
                              
abundance_df <- instrain_breadth_depth %>% select() %>%
