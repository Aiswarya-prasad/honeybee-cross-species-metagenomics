...
# save.image("results/figures/workspace_generaldata_chunks_20230611.RData")
...
# DO NOT exclude low coverage samples here, exclude later
...
Code for exploration not included in `.Rdata` file.
...
vis_magOTUs_df %>%
  ...
  filter(Completeness > 97) %>%
  # filter(Genus != "g__") %>%
  # pull(ID) %>%
    # unique %>% length
  write.csv("/scratch/aprasad/Bombilactobacillus_highquality_MAGs_metadata.csv", row.names = F, quote = F)
mags_to_copy <- vis_magOTUs_df %>%
  left_join(handmade_species_names %>% select(cluster, MAG_species_name_final), by = c("magOTU" = "cluster")) %>%
  # filter(Genus %in% c(
    # "g__Commensalibacter"#,
    # "g__Saezia",
...
system("mkdir -p /scratch/aprasad/Bombilactobacillus_highquality_MAGs")
for (mag in mags_to_copy) {
  system(paste0("cp /work/FAC/FBM/DMF/pengel/spirit/aprasad/BACKUP_current/20230313_apis_species_comparison/results/09_MAGs_collection/MAGs_high_medium/", mag, ".fa /scratch/aprasad/Bombilactobacillus_highquality_MAGs"))
}

vis_magOTUs_df %>%
  filter(grepl("apicola_K", Species)) %>%
  # filter(grepl("zhangqiuensis", Species)) %>%
  # filter(magOTU == "74_1") %>%
    select(ID, Completeness, Size, Representative, magOTU)
