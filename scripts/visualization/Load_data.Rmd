...
for (sample in samples) {
  if (file.exists(paste0("results/01_trimmedconcatreads/", sample, "_R1.fastq.gz"))) {
    raw_R1_reads <- paste0("results/01_trimmedconcatreads/", sample, "_R1.fastq.gz")
    number_raw_R1 <- system(paste("zcat ", raw_R1_reads, " | grep -e '^+$' | wc -l"), intern = T)
  } else {
    number_raw_R1 <- NA
  }
  if (file.exists(paste0("results/01_trimmedconcatreads/", sample, "_R2.fastq.gz"))) {
    raw_R2_reads <- paste0("results/01_trimmedconcatreads/", sample, "_R2.fastq.gz")
    number_raw_R2 <- system(paste("zcat ", raw_R2_reads, " | grep -e '^+$' | wc -l"), intern = T)
  } else {
    number_raw_R2 <- NA
  }
  if (file.exists(paste0("results/00_trimmedreads/fastqc/", sample, "_R1_trim_fastqc.zip"))) {
    trimmedconcat_R1_reads <- paste0("results/01_trimmedconcatreads/", sample, "_R1.fastq.gz")
    number_trimmedconcat_R1 <- system(paste("zcat ", trimmedconcat_R1_reads, " | grep -e '^+$' | wc -l"), intern = T)
  } else {
    number_trimmed_R1 <- NA
  }
  if (file.exists(paste0("results/00_trimmedreads/fastqc/", sample, "_R2_trim_fastqc.zip"))) {
    trimmedconcat_R2_reads <- paste0("results/01_trimmedconcatreads/", sample, "_R2.fastq.gz")
    number_trimmedconcat_R2 <- system(paste("zcat ", trimmedconcat_R2_reads, " | grep -e '^+$' | wc -l"), intern = T)
  } else {
