...
# only an example - update paths later
# make matrix getter to return matrix and the annotation info as list onject 1 & 2
get_motupan_matrix <- function(my_genus = "g__Lactobacillus", combined_info = combined_info_df) {
  motupan_counts <- paste0("/scratch/aprasad/211018_Medgenome_india_samples/15_FurtherProcessing/motupan_test/",my_genus,"/mOTUpan_gene_counts.tsv")
  ...
  motupan_df <- read.csv(motupan_out, skip = 16, sep = "\t") %>%
                  ...
                  select(type) %>%
                  rownames_to_column("OG")
  OG_numbers_df <- read.csv(motupan_counts, sep = "\t") %>%
  ...
  matrix_arranged <- OG_numbers_df %>%
                      t() %>%
                          ...
                          left_join(vis_magOTUs_df %>%
                            ...
                            column_to_rownames("ID") %>% 
                            as.matrix
  return(list("matrix" = matrix_arranged, "info" = types_df))
}
# matrix = get_motupan_matrix("g__Lactobacillus", combined_info_df)$matrix
# motupan_info <- get_motupan_matrix("g__Lactobacillus", combined_info_df)$info
# names(motupan_info)
