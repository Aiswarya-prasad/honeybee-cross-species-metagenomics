...
sample = samples_IN[1]
instrain_mags_ref <- read.csv(paste0("/scratch/aprasad/211018_Medgenome_india_samples/10_instrain/",sample,"_profile.IS/output/",sample,"_profile.IS_mapping_info.tsv"), sep = "\t", skip = 1) %>%
      mutate(ID = Vectorize(get_ID_from_scaffold)(scaffold)) %>%
      filter(ID != "all_scaffolds") %>%
      pull(ID) %>% unique() %>% as.character
instrain_mag_lengths <- read.csv(paste0("/scratch/aprasad/211018_Medgenome_india_samples/10_instrain/",sample,"_profile.IS/output/",sample,"_profile.IS_scaffold_info.tsv"), sep = "\t") %>%
      mutate(ID = Vectorize(get_ID_from_scaffold)(scaffold)) %>%
      select(ID, length) %>%
      group_by(ID) %>%
      summarise(ID, length = sum(length)) %>%
      unique()
instrain_mapping <- data.frame(ID = instrain_mags_ref)
for (sample in samples_IN) {
  sample_reads <- read.csv(paste0("/scratch/aprasad/211018_Medgenome_india_samples/10_instrain/",sample,"_profile.IS/output/",sample,"_profile.IS_mapping_info.tsv"), sep = "\t", skip = 1) %>%
    mutate(ID = Vectorize(get_ID_from_scaffold)(scaffold)) %>%
      select(ID, filtered_pairs) %>%
      filter(ID != "all_scaffolds") %>%
      group_by(ID) %>%
      summarise(ID, reads=sum(filtered_pairs)) %>%
      unique() %>% pull(reads)
  sample_MAGs <- read.csv(paste0("/scratch/aprasad/211018_Medgenome_india_samples/10_instrain/",sample,"_profile.IS/output/",sample,"_profile.IS_mapping_info.tsv"), sep = "\t", skip = 1) %>%
      mutate(ID = Vectorize(get_ID_from_scaffold)(scaffold)) %>%
      filter(ID != "all_scaffolds") %>%
      pull(ID) %>% unique() %>% as.character
  print(sample)
  print(sum(sample_reads))
  df_to_bind <- data.frame(sample_MAGs, sample_reads)
  colnames(df_to_bind) <- c("ID", sample)
  instrain_mapping <- cbind(instrain_mapping, df_to_bind %>% select(!ID))
} 

plot_mapping_df_by_magotu <- instrain_mapping %>%
                              pivot_longer(!ID, values_to = "reads", names_to = "sample") %>%
                              left_join(vis_magOTUs_df_all %>% 
                                          ungroup() %>%
                                          # filter(Ref_status == 1) %>%
                                          mutate(magOTU = paste0(Genus, "_", Cluster)) %>%
                                          select(ID, magOTU, Genus, length)
                              ) %>%
                              left_join(instrain_mag_lengths) %>%
                              mutate(reads_corrected = reads*150*2/length) %>%
                              group_by(sample) %>%
                              mutate(percentage_corrected = reads_corrected/sum(reads_corrected)*100) %>%
