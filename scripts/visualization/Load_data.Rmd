...
abundance_df <-  instrain_breadth_depth_unfiltered %>%
                                filter(!is.na(magOTU)) %>%
                                left_join(df_reads %>% select(Sample, MAGs_DB), by = c("sample" = "Sample")) %>%
                                  mutate(coverage = coverage*MAGs_DB/min_reads) %>%
                                  mutate(coverage = coverage*Vectorize(qpcr_normalizing_factor)(Host)) %>%
                                    select(!MAGs_DB) %>%
                                      unique()
...
# df_Cov_copy_num_adj <-  abundance_df %>% filter(!is.na(magOTU)) %>%
#                           select(Sample, magOTU, Cov) %>%
#                           left_join(cluster_tax_info %>% ungroup %>% select(Genus, magOTU)) %>%
...
#                             mutate(Host = Vectorize(get_host_from_sample_name)(Sample))
# abundance_df_matrix_Cov <-  abundance_df %>% filter(!is.na(magOTU)) %>%
#                           select(Sample, magOTU, Cov) %>%
#                           # filter(Sample %in% samples_IN) %>%
#                           unique() %>%
#                             pivot_wider(names_from=magOTU, values_from=Cov, values_fn = as.numeric) %>% remove_rownames %>%
#                             column_to_rownames(var="Sample")
