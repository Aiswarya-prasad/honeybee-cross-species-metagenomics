...
# only an example - update later
# motu_pan_out <- "/scratch/aprasad/211018_Medgenome_india_samples/15_FurtherProcessing/motupan_test/g__Lactobacillus_with_refs/mOTUpan.tsv"
# get_motu_pan_matrix <- function(motu_pan_out, scp = F) {
#   df_motu <- read.csv(motu_pan_out, skip = 16, sep = "\t")
#   # genome_occurences is the number of genomes it occurs in
#   # mean_copy_per_genome is the number of times it occurs in each of them
#   # single copy genomes should have mean_copy_per_genome value of exactly 1
#   if (scp) {
#     names(df_motu)
#     df_motu_mat <- df_motu %>%
#                     mutate(mean_copy_per_genome = as.numeric(mean_copy_per_genome)) %>%
#                     filter(mean_copy_per_genome == 1) %>%
#                       select(trait_name, genes) %>% head
#   } else {
#     df_motu_mat <- df_motu
#   }
# }
...
# Load above data without changes
