...
p_list <- list()
for (a_feature in unique(cazy_df$feature)) {
  p <- ggplot(cazy_df %>% filter(feature == a_feature)) +
    geom_boxplot(aes(y = uniq_rpkm, x = Host, fill = Host), outlier.size = 0) +
    geom_point(aes(y = uniq_rpkm, x = Host, fill = Host), alpha = 0.5) +
    ggtitle(a_feature) +
    labs(y = "CAZy GH family", x = "rpkm", fill = "Host") +
    scale_fill_manual(values = host_order_color) +
    make_theme(setFill = FALSE, leg_pos = "none")
  p_list[[a_feature]] <- p
}
pdf("results/figures/cazy_boxplots.pdf")
marrangeGrob(p_list, nrow = 2, ncol = 2)
dev.off()

"data/cayman_gene_db/20230607_glycan_annotations_cleaned.xlsx"
...
cazy_df_annotated <- cazy_df %>%
                      ...
                      mutate(short_names = abbreviate(FUNCTION_AT_DESTINATION_3, minlength = 30))
...
column_split <- colnames(cazyme_uniq_mat_pec) %>% as.data.frame() %>% mutate(Host = Vectorize(get_host_from_sample_name)(.)) %>% pull(Host) %>% as.factor %>% as.numeric
row_split <- rownames(cazyme_uniq_mat_pec) %>% as.data.frame() %>% left_join(cazy_df_annotated %>% select(feature, short_names) %>% filter(!is.na(short_names)) %>% unique, by = c("." = "feature")) %>% pull(short_names) %>% replace_na(., "Unknown") %>% as.factor %>% as.numeric
...
pdf("results/figures/cazyme_uniq_rpkm.pdf", width = 15, height = 25)
draw(heatmap_obj, merge_legend = TRUE)
dev.off()


# make_without unknown
known_families <- cazy_df_annotated %>% select(feature, FUNCTION_AT_DESTINATION_3) %>% unique %>% filter(!is.na(FUNCTION_AT_DESTINATION_3)) %>% filter(FUNCTION_AT_DESTINATION_3 != "Unknown") %>% pull(feature)
...
column_split <- colnames(cazyme_uniq_mat_pec_no_na) %>% as.data.frame() %>% mutate(Host = Vectorize(get_host_from_sample_name)(.)) %>% pull(Host) %>% as.factor %>% as.numeric
row_split <- rownames(cazyme_uniq_mat_pec_no_na) %>% as.data.frame() %>% left_join(cazy_df_annotated %>% select(feature, short_names) %>% filter(!is.na(short_names)) %>% unique, by = c("." = "feature")) %>% pull(short_names) %>% replace_na(., "Unknown") %>% as.factor %>% as.numeric
col_fun = colorRamp2(c(0, 0.1, 1, 10, 1000), c(brewer.pal(9, "PuBu")[1], brewer.pal(9, "PuBu")[3], brewer.pal(9, "PuBu")[4], brewer.pal(9, "PuBu")[7], brewer.pal(9, "PuBu")[9]))
