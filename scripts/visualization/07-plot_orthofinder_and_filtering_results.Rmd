...
for (genus_iter in genera) {
  my_upper_count <- 20
  if (genus_iter == "g__Lactobacillus") {
    pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_counts.pdf"))
    my_heatmap <- pretty_orthogene_heatmaps(matrix = get_genecount_matrix(my_genus=genus_iter, upper_count = my_upper_count)$matrix, 
      do_motupan_anno = T, motupan_info = get_genecount_matrix(my_genus=genus_iter, upper_count = my_upper_count)$info,
    ...
    pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_scp.pdf"))
      my_heatmap <- pretty_orthogene_heatmaps(matrix = get_genecount_matrix(my_genus=genus_iter, scps_only = T, upper_count = my_upper_count)$matrix, 
      do_motupan_anno = T, motupan_info = get_genecount_matrix(my_genus=genus_iter, scps_only = T, upper_count = my_upper_count)$info,
      col_fun = colorRamp2(c(0, 1, 2, 5, my_upper_count), c("#e6f5d0", "#7fbc41", brewer.pal(9, "RdPu")[1], brewer.pal(9, "RdPu")[8], brewer.pal(9, "RdPu")[9])))
    draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
    dev.off()
  } else {
...
my_heatmap <- pretty_orthogene_heatmaps(matrix=get_motupan_matrix(scps_only = F, my_genus=genus_iter)$matrix,
     do_motupan_anno = T, motupan_info = get_motupan_matrix(scps_only = F, my_genus=genus_iter)$info,
     col_fun = colorRamp2(c(0, 1, 2, 5, my_upper_count), c("#e6f5d0", "#7fbc41", brewer.pal(9, "RdPu")[1], brewer.pal(9, "RdPu")[8], brewer.pal(9, "RdPu")[9])))
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
my_heatmap <- pretty_orthogene_heatmaps(matrix=get_motupan_matrix(scps_only = T, my_genus=genus_iter)$matrix,
     do_motupan_anno = T, motupan_info = get_motupan_matrix(scps_only = T, my_genus=genus_iter)$info,
     col_fun = colorRamp2(c(0, 1, 2, 5, my_upper_count), c("#e6f5d0", "#7fbc41", brewer.pal(9, "RdPu")[1], brewer.pal(9, "RdPu")[8], brewer.pal(9, "RdPu")[9])))
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
# Do this for all the genera (with motupan and add these annotations for orthofinder results)
# Check for presence (core-ness) at the magOTU-level too also while filtering single copy cores
```
