...
combined_info_df <- vis_magOTUs_df_all %>%
                      select(colnames(reference_genomes_info)) %>%
                        rbind(reference_genomes_info)

for (genus_iter in genera) {
  pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_counts.pdf"))
  my_upper_count <- 20
  my_heatmap <- pretty_orthogene_heatmaps(get_genecount_matrix(my_genus=genus_iter, upper_count = my_upper_count), 
  ...
  dev.off()
  pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_scp.pdf"))
  my_heatmap <- pretty_orthogene_heatmaps(get_scp_matrix(my_genus=genus_iter), 
     col_fun = brewer.pal(9, "RdPu")[c(2,4)])
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("All single copy genes", "Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
  dev.off()
  pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_isolate_core.pdf"))
  my_heatmap <- pretty_orthogene_heatmaps(get_isolate_core_matrix(my_genus=genus_iter), 
      col_fun = brewer.pal(9, "RdPu")[c(2,4)])
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Core in isolates and present in some mags", "Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
  dev.off()
  pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_mag_half_core.pdf"))
  my_heatmap <- pretty_orthogene_heatmaps(get_half_core_mag_matrix(my_genus=genus_iter), 
      col_fun = brewer.pal(9, "RdPu")[c(2,4)])
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("All single copy genes", "Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
  dev.off()
  pdf(paste0("Figures/07-Orthogroup_heatmaps/", genus_iter, "_OG_mag_half_core_mags_only.pdf"))
  my_heatmap <- pretty_orthogene_heatmaps(get_half_core_mag_matrix(my_genus=genus_iter, only_mags = T, with_isolates = F), 
      col_fun = brewer.pal(9, "RdPu")[c(2,4)])
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Genes present in at least 50% of the MAGs", "Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
  dev.off()
}
```
...
my_heatmap <- pretty_orthogene_heatmaps(get_motupan_matrix(my_genus=genus_iter)$matrix,
     do_motupan_anno = T, motupan_info = get_motupan_matrix(my_genus=genus_iter)$info,
     col_fun = colorRamp2(c(0, 1, 2, 5, my_upper_count), c("#e6f5d0", "#7fbc41", brewer.pal(9, "RdPu")[1], brewer.pal(9, "RdPu")[8], brewer.pal(9, "RdPu")[9])))
  draw(my_heatmap, merge_legend = TRUE, column_title=paste0("Number of OG genes: ", ncol(my_heatmap)), column_title_gp=grid::gpar(fontsize=12))
