...
df_rohdes_by_host <- df_rohdes %>%
                      ...
                      # filter(rank == "1") %>%
                      # filter(X_old %in% (prevs_df %>% filter(prevalence_in_host >= 0.04) %>% pull(X)))

ggplot(df_rohdes_by_host,
  ...
  labs(y = "Rohde's index", x = "Host species", fill = "Host") +
...
for (genus in unique(df_plot$Genus)) {
  p <- ggplot(df_plot %>% filter(Genus == genus) %>% group_by(sample) %>% mutate(rel_cov = rel_cov/sum(rel_cov))) +
    ...
    make_theme(theme_name=theme_few(), leg_pos="right",
               ...
               x_angle=45 ,x_vj=1, x_hj=1, x_size=5,
               y_angle=0 ,y_vj=0, y_hj=0, y_size=10)
  ggsave(paste0("results/figures/Species_Relative_abundance/",genus, "_spec_rel_abund.pdf"), plot = p, width = 20, height = 15)
  p_list[[genus]] <- p
}
...
ggplot(df_phy_dist_same_spec,
       aes(x = Host_dist, y = Bact_dist)) +
  geom_point() +
  facet_wrap(~Species1, scales = "free") +
  geom_smooth(method = "lm", se = T) +
  labs(x = "Host divergence", y = "Bacteria divergence") +
  make_theme(theme_name=theme_few(), leg_pos="none",
             # guide_nrow = 6,
             setFill = F, setCol = F,
             x_angle=45 ,x_vj=1, x_hj=1, x_size=5,
...
ggplot(df_phy_dist,
       aes(x = Host_dist, y = Bact_dist)) +
  geom_point() +
  facet_wrap(~spec_comb, scales = "free") +
  geom_smooth(color = "#000000", method = "lm", formula = y ~ x, se = F) +
  labs(x = "Host divergence", y = "Bacteria divergence") +
  make_theme(theme_name=theme_few(), leg_pos="top",
             # guide_nrow = 6,
             setFill = F, setCol = F,
             x_angle=45 ,x_vj=1, x_hj=1, x_size=5,
             y_angle=0 ,y_vj=0, y_hj=0, y_size=10)
