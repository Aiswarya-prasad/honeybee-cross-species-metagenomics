...
ggplot(coverage_df_genus %>% filter(Sample %in% samples), aes(y = factor(Genus, genera), x = factor(Host, host_order), fill = Mean)) +
                            geom_tile() +
                              labs(x = "Sample", y = "Cluster")+
                              # scale_fill_gradient(na.value = "transparent", low = "#ffffff" , high = "#1a88c9", guide = "colourbar", trans = "log10") +
                              scale_fill_gradient2(midpoint = 40, low = "#ffffd9",  mid = "#41ab5d", high = "#006837", guide = "colourbar", na.value = "#ffffd9") +
                                make_theme(setFill = F, modify_guide = F, y_size = 8,
                                  x_size = 10, x_angle = 30, x_hj = 1 , x_vj = 1,
                                  y_hj =1, leg_pos = "right"
                                )
                                ggsave("Figures/09-mean_rel_abundance_from_corecov.pdf")
        ...
        aes(y = factor(Genus, genera), x = Cov,, color =  factor(Genus, genera))) +
                            ...
                            # geom_text(aes(label = Mean)) +
                              ...
                              scale_color_manual(values = genusColors) +
                                ...
                                ggsave(paste0("Figures/", "09-Coverage_scatter_genus_across_host.pdf"))
...
ggplot() +
  ...
  ggsave("Figures/09-Abundance_by_sample_qPCR_adj.pdf")
...
pdf("Figures/09-PCoA_sorensen_microbiome.pdf")
...
# pdf("Figures/09-PCoA_jaccard_microbiome.pdf")
...
ggplot() +
  ...
  geom_violin(data=dissimilarities_df,
    ...
    ggplot("Figures/09-Microbe_host_dissimilarities_violin.pdf")
...
ggplot() +
  ...
  ggplot("Figures/09-Microbe_host_dissimilarities.pdf")
...
plot_tree(ps, nodelabf=nodeplotblank, color="Host", label.tips = "Name", ladderize="left", plot.margin=0.3) + 
  scale_color_manual(values = host_order_color)
  ggsave("Figures/09-Plot_bac120_tree_with_host.pdf")
