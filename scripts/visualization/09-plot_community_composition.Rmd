...
instrain_breadth_depth_unfiltered_all_handmade <- instrain_breadth_depth_unfiltered_all %>% 
...
# implemented this in python scripts/visualization/host_specificity_of_community.py and made "results/figures/magotu_rohdes_index.csv"
...
df_plot <- abundance_df %>% filter(!is.na(magOTU)) %>%
                            ...
                            left_join(handmade_species_names %>% select(MAG_species_name_final, cluster_name_instrain), by = c("magOTU" = "cluster_name_instrain"))
write.csv(df_plot, "results/figures/df_plot_annotated.csv", row.names = F, quote = F)
...
# Use prevalence, it seems to be the most robust... 
...
ggplot() +
  ...
  scale_alpha_manual(values=c("1" = 1,
    ...
    make_theme(leg_pos = "right", setFill = F, setCol = F, x_angle = 30, x_vj = 1, x_hj = 1, 
      ...
      ggsave("results/figures/09-figures/3-magotu_rohdes_by_magotu_across.pdf", width = 12, height = 15)
...
df_scatter_pie <- abundance_df %>% filter(!is.na(magOTU)) %>%
                            ...
                            # fixing mislabelled names
                            mutate(magOTU = case_when(
                              magOTU == "NA_NA-F5-1_7" ~ "Bifidobacterium_polysaccharolyticum-F5-1_7",
                              magOTU == "NA_NA-F4-5_12" ~ "Gilliamella_apicola_K-F4-5_12",
                              magOTU == "NA_NA-F4-5_6" ~ "Apilactobacillus_zhangqiuensis-F4-5_6",
                              magOTU == "NA_NA-F4-5_22" ~ "Snodgrassella_SGB-58_1-F4-5_22",
                              TRUE ~ magOTU
                            )) %>%
                            mutate(MAG_species_name_final = case_when(
                              magOTU == "Bifidobacterium_polysaccharolyticum-F5-1_7" ~ "Bifidobacterium polysaccharolyticum",
                              magOTU == "Gilliamella_apicola_K-F4-5_12" ~ "Gilliamella apicola_K",
                              magOTU == "Apilactobacillus_zhangqiuensis-F4-5_6" ~ "Apilactobacillus zhangqiuensis",
                              magOTU == "Snodgrassella_SGB-58_1-F4-5_22" ~ "Snodgrassella SGB-58_1",
                              TRUE ~ MAG_species_name_final
                            )) %>%
                            mutate(Genus = case_when(
                              MAG_species_name_final == "Bifidobacterium polysaccharolyticum" ~ "g__Bifidobacterium",
                              MAG_species_name_final == "Gilliamella apicola_K" ~ "g__Gilliamella",
                              MAG_species_name_final == "Apilactobacillus zhangqiuensis" ~ "g__Apilactobacillus",
                              MAG_species_name_final == "Snodgrassella SGB-58_1" ~ "g__Snodgrassella",
                              TRUE ~ Genus
                            )) %>%
                            select(-magOTU) %>%
...
view(df_scatter_pie %>% mutate(rowsum = `Apis mellifera` + `Apis cerana` + `Apis dorsata` + `Apis florea` + `Apis andreniformis`))
df_scatter_pie(filter())
...
df_scatter_pie_plot <- df_scatter_pie %>%
                        # mutate(Species = as.numeric(Species)) %>%
                            ...
                            # # randomly add or subtract a number between 0 and 0.75 from each entry in Genus column
                            mutate(Genus = Genus + runif(length(Genus), -0.75,0.75))
...
ggplot() +
  ...
  coord_equal()
  ggsave("results/figures/09-figures/3-magotu_rohdes_scatter_pie_radius.pdf", width = 15, height = 10)
...
ggplot(plot_corr_species_num, aes(x = MAGs_DB_mf, y = rowsum, color = Host)) +
  ...
  make_theme(leg_pos = "bottom", setCol = F,
             axis_x_title = 16, axis_y_title = 16,
             guide_nrow = 3)
  ggsave("results/figures/09-Correlation_species_num_reads_by_host_all_locations.pdf")

df_rowsums = data.frame(rowSums(abundance_df_matrix_pa)) %>%
              rownames_to_column("Sample")
colnames(df_rowsums) = c("Sample", "rowsum")
df_rowsums_cov = data.frame(rowSums(abundance_df_matrix)) %>%
              rownames_to_column("Sample")
colnames(df_rowsums_cov) = c("Sample", "rowsum_cov")
plot_corr_species_num <- df_reads %>%
                          select(Sample, MAGs_DB_mf) %>%
...
ggplot(plot_corr_species_num, aes(x = MAGs_DB_mf, y = rowsum, color = Host)) +
  ...
  scale_color_manual(values=host_order_color_dark) +
  scale_x_continuous(labels=unit_format(unit = "M", scale = 1e-6)) +
  labs(x = "Number of reads mapping to MAGs", y = "Number of species clusters detected") +
  facet_wrap(~Host) +
  make_theme(leg_pos = "bottom", setCol = F,
             axis_x_title = 16, axis_y_title = 16,
             guide_nrow = 3)
  ggsave("results/figures/09-Correlation_species_num_reads_by_host_IN_MY.pdf")
...
colnames(df_rowsums_cov) = c("Sample", "rowsum_cov")
plot_corr_species_num <- df_reads %>%
                          mutate(nonhost = Total_clean - Host_mapped_hf) %>%
                          select(Sample, nonhost) %>%
                            left_join(df_rowsums) %>%
                            left_join(df_rowsums_cov) %>%
                            mutate(Host = Vectorize(get_host_from_sample_name)(Sample)) %>%
                            mutate(Location = Vectorize(get_location_from_sample_name)(Sample))

ggplot(plot_corr_species_num, aes(x = nonhost, y = rowsum, color = Host)) +
  stat_cor(digits = 3, size = 5) +
  geom_smooth(method = "lm", se = TRUE) +
  geom_point(aes(shape = Location), size = 3) +
  scale_color_manual(values=host_order_color_dark) +
  scale_x_continuous(labels=unit_format(unit = "M", scale = 1e-6)) +
  labs(x = "Number of non-host reads in sample", y = "Number of species clusters detected") +
  facet_wrap(~Host) +
  make_theme(leg_pos = "bottom", setCol = F,
             axis_x_title = 16, axis_y_title = 16,
             guide_nrow = 3)
  ggsave("results/figures/09-Correlation_species_num_nonhost_reads_by_host_IN_MY_se.pdf")
...
colnames(df_rowsums_cov) = c("Sample", "rowsum_cov")
plot_corr_species_num <- df_reads %>%
                          select(Sample, Total_clean) %>%
                            left_join(df_rowsums) %>%
                            left_join(df_rowsums_cov) %>%
                            mutate(Host = Vectorize(get_host_from_sample_name)(Sample)) %>%
                            mutate(Location = Vectorize(get_location_from_sample_name)(Sample))

ggplot(plot_corr_species_num, aes(x = Total_clean, y = rowsum, color = Host)) +
  stat_cor(digits = 3, size = 5) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_point(aes(shape = Location), size = 3) +
  scale_color_manual(values=host_order_color_dark) +
  scale_x_continuous(labels=unit_format(unit = "M", scale = 1e-6)) +
  labs(x = "Total number of reads", y = "Number of species detected") +
  facet_wrap(~Host) +
  make_theme(leg_pos = "bottom", setCol = F,
             axis_x_title = 16, axis_y_title = 16,
             guide_nrow = 3)
...
system("mkdir -r results/figures/09-presence_absence_heatmap_cluster_sep_rel")
...
ggplot(df_box_plot) +
  ...
  make_theme(leg_pos = "none", setCol = F, setFill = F,
             axis_x_title = 16, axis_y_title = 16,
             guide_nrow = 1)
  ggsave("results/figures/09-figures/09-Species_per_sample_boxplot.pdf")
...
df_magOTUs_vegan_pa_no_adre_melli <- abundance_df_matrix_pa %>%
                      filter(rowSums(.) > 0) %>%
                      filter(!rownames(.) %in% samples_to_exclude) %>%
                      filter(!rownames(.) %in% samples_AA) %>%
                      filter(!rownames(.) %in% samples_AM)
# df_pcoa <- dist_matrix_betapart$beta.sor
dist_matrix_betapart <- beta.pair(df_magOTUs_vegan_pa_no_adre_melli, "sorensen")
# test dispertion and adonis2
dist_obj <- dist_matrix_betapart$beta.sor
ado_res = adonis2(dist_obj ~ Location, 
        rownames(dist_obj %>% as.matrix) %>% as.data.frame() %>%
...
# adonis_OmegaSq(ado_res)
pdf("results/figures/09-figures/09-PCoA_sorensen_microbiome_turnover.pdf", width = 10, height = 15)
...
pdf("results/figures/09-PCoA_sorensen_microbiome_location_as_shape.pdf", width = 10, height = 15)
pcoa_plot(dist_matrix_betapart$beta.sor, df_meta_complete %>% select(ID, Species, Country), variable = "Species", shape_add=T, shape_var = "Country", color_add=T, color_list=host_order_color_dark)
dev.off()
...
ggplot() +
  ...
  # ggsave("results/figures/09-figures/09-Microbe_host_dissimilarities_sorenson_turnover_color.pdf", width = 15, height = 15)
