...
```{r}
ggplot(instrain_prev_by_genus, aes(y = factor(Genus, genera_ext), 
...
allu_df <- instrain_prev %>% 
            ...
            summarise(magOTU, Host, Genus, numSamples = sum(detected)) %>%
            filter(!is.na(Genus)) %>%
            unique()

ggplot(allu_df %>% filter(numSamples >= 5),
      aes(axis1 = magOTU, axis2 = factor(Host, host_order), y = numSamples)) +
  geom_alluvium(aes(fill = factor(Genus, genera_ext)), color = "black", size = .5) +
  ggtitle("Host range of magOTUs (detected in >= 5 samples of a given host)") +
  scale_fill_manual(values=genusColors_char) +
  ...
  theme(axis.line=element_blank(),
      ...
      ggsave("results/figures/3-host_range_alluvial_filt.pdf")
