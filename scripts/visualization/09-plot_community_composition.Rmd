...
paginate_save(plot_scaff_detected, "magOTU", "results/figures/10-instrain_plots/1-scaffolds_detected_by_magOTU.pdf", pass_nrow = 3, pass_ncol = 3)
```

```{r instrain_plots_breadth_by_sample}
colnames(instrain_genomes_info_df)
...
paginate_save(plot_breadth_by_sample_w_cov, "sample", "results/figures/10-instrain_plots/1-breadth_vs_expected_w_cov_by_sample.pdf", pass_nrow = 3, pass_ncol = 3)
```
...
# plot_magotu_shared <- ggplot(df_all_combos_plot) +
#   geom_point(aes(y = factor(host_comp, host_comp_order), x = prop_shared), color = "#000000", size = 2.2) +
#   geom_point(aes(y = factor(host_comp, host_comp_order), x = prop_shared, color = Genus), size = 2) +
#     labs(x = "Host species pair", y = "Proportion of host species pairs that share the magOTU") +
#     scale_color_manual(values=genusColors) +
#     facet_wrap_paginate(~magotu, ncol = 4, nrow = 4, page = 3) +
#     make_theme(setFill = F, setCol = F,
#                y_hj = 1, y_vj = 1, y_size = 10, y_angle = 30,
#                leg_pos = "none"
#     ) +
#       theme(strip.text = element_text(size = 10),
...
# paginate_save(plot_magotu_shared, "magotu", "results/figures/3-magotu_shared.pdf", pass_nrow = 4, pass_ncol = 4)
...
# rhodes specificity index

# for each host species j, sum_j()
