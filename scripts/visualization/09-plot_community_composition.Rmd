...
```{r community_composition_genus}
ggplot() +
  geom_bar(data = plot_mapping_df_by_magotu,
           aes(x = reads,
               y = sample,
               fill = Genus
           ),
  ...
  labs(y = "Number of reads (corrected)", x = "Sample", fill = "Genus") +
  ...
  make_theme(theme_name=theme_few(), leg_pos="bottom",
                                         guide_nrow = 10,
                                         setFill = F, max_colors = length(unique(plot_mapping_df_by_magotu$Genus)),
                                         x_angle=45 ,x_vj=1.2, x_hj=1, x_size=12,
                                         y_angle=0 ,y_vj=0, y_hj=0, y_size=12) +
    scale_x_continuous(labels=unit_format(unit = "M", scale = 1e-6))
    ggsave("Figures/09-Genus_composition_by_instrain_mapping_num.pdf")
ggplot() +
  geom_bar(data = plot_mapping_df_by_magotu,
           aes(x = reads_corrected,
               y = sample,
               fill = Genus
           ),
           stat = "identity", position = "stack"
