...
=head1 SYNOPSIS
​
perl calc_jaccard.pl firm4_1_filt.freq
​
=head1 DESCRIPTION
​
This script will calculate jaccard indices (i.e. the shared fraction) of variants between pairs of samples.
​
Only presence of alleles is considered (not absences, which are far more frequent). Thus, if both samples contain a variant, its a share, if one of them contain the variant is a non-share, if none of them have the variant its not counted.
​
=cut
​
GetOptions (
