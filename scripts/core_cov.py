...
requiredNamed.add_argument('-d',metavar="db_metafile",required=True, help="File detailing genome-id and SDP affiliation", action="store")
requiredNamed.add_argument('-g', metavar='orthofinder_file', required=True, help="Filtered single-copy ortholog gene file (orthofinder format)", action="store")
requiredNamed.add_argument('-b', metavar="bedfile_dir", required=True, help="Directory containing bed-files for genomes in db", action="store")
group_list = parser.add_mutually_exclusive_group(required=True)
group_list.add_argument('-l', metavar="bamfile_list", help="List of bam-files, one line per file", action="store")
group_list.add_argument('-L', metavar="bamfile_list_explicit", nargs="+", help="List of bam-files as a list in the command line", action="store")
parser.add_argument('-o', metavar="out_dir", required=False, help="Directory to write outputs to", action="store")
parser.add_argument('-n', metavar="out_name", required=False, help="name to append to output file", action="store")
...
if (args['L']):
    #Read the bam-files list, save files in list
    bamfiles_explicit = list()
    # # check if argparse handles a list like this!
    for file in bamfile_list_explicit:
        file = os.path.join(os.getcwd(), file)
        check_file_exists(file)
        bamfiles_explicit.append(file)
    bamfiles = bamfiles_explicit
