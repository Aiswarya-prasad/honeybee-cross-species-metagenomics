...
x = filt_df_list$"M1.1_microbiome_mapped"
x
...
data <- read.table("/Volumes/Storage/Work/Temp-From-NAS/firm5_corecov.txt",h=T)
...
outfile_prefix = "/Volumes/Storage/Work/Temp-From-NAS/firm5"
...
for (sdp in sdps) {
    ...
    #Get fitted coordinates and append values to coo

    filt_df_listrd-table. Plot if data allows (and if user want..)
    sdp_coord <- lapply(filt_df_list, coord_and_plot, 1)
