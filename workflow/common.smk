...
def handmade_species_name(magotu, column='MAG_species_name_final_nospace'):
    handmade_spec_names = pd.read_csv('config/Species_MAG_Cluster-names.txt', sep='\t', header=0)
    # contains: MAG_species_name_final	MAG_species_Name_in_analysis	cluster	RepMAG	MAG_species_name_final_nospace
    if magotu in handmade_spec_names['cluster'].values:
        species = handmade_spec_names[handmade_spec_names['cluster'] == magotu][column].values[0]
    else:
        species = magotu
    return(species)

def get_mag_info(mag, metadata, column='magOTU'):
    with open(metadata, 'r') as f:
        header = f.readline()
        header = header.strip()
        id_ind = header.split('\t').index('ID')
        ind = header.split('\t').index(column)
        for line in f:
            if line.split('\t')[id_ind] == mag:
